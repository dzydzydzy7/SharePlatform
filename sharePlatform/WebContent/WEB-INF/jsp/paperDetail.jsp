...
<body>
	...
	<div class="layui-col-md12">
		<div class="layui-card">
			<div class="layui-card-header"
				style="font-weight: bold; font-size: 24px;">${paper.title }</div>
			<div class="layui-card-header">作者：${paper.author}&emsp;&emsp;
				领域：${paper.field }&emsp;&emsp; 期刊：${paper.journal}&emsp;&emsp;
				上传时间：${paper.uploadtime }&emsp;&emsp; 被下载次数：${paper.downtimes }</div>
			<div class="layui-card-body">摘要：${paper.abstarct }</div>
			<div class="layui-card-header">
				相关的代码：
				<c:forEach items="${codes }" var="code">
					<a href="<c:url value="/source/code?id=${code.id }"/>">${code.name }</a>
					&emsp;
				</c:forEach>
			</div>
			<div class="layui-card-header">
				...
				<c:forEach items="${datasets }" var="dataset">
					<a href="<c:url value="/source/dataset?id=${dataset.id }"/>">${dataset.name }</a>
					&emsp;
				</c:forEach>
			</div>
			<div class="layui-card-body">
				<form method="post" name="theForm"
					action="${pageContext.request.contextPath }/source/downPaper?id=${paper.id}">
	...
	<form method="post" action="${pageContext.request.contextPath }/source/commentPaper?id=${paper.id }">
		<div class="layui-row">
